...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    description: item.description ?? `View our ${item.title} website portfolio project.`,
    alternates: { canonical: `/portfolio/websites/${params.slug}` },
    openGraph: {
      title: `${item.title} | OctaNex31 Portfolio`,
      description: item.description ?? `View our ${item.title} website portfolio project.`,
...
export default async function PortfolioWebsiteDetailPage({ params }: Props) {
  const item = await getPortfolioItem(params.slug);
  if (!item) notFound();
